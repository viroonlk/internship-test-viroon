...
- **วิธีแก้:** ค่าที่เกี่ยวกับสิทธิ์ (Role/ID) ไม่ควรรับจาก Body แต่ควรดึงมาจาก Middleware ที่ทำหน้าที่ยืนยันตัวตน (เช่น ตรวจสอบจาก JWT Token และเก็บไว้ใน `req.user`) 
